...
void addFolder(FOLDER** parent, FOLDER* subdir) {
    if (parent[0]->subcount == 0) {
        ...
        parent[0]->capacity *= 2;
        printf("Reallocating more memory: %d", (int)parent[0]->capacity);
...
int main () {
    ...
    //Temporary, the final program will have you choose the two resourcepacks for scan
    rp->subdir = scanFolder(rp, path);
    getchar();
    free(path);
